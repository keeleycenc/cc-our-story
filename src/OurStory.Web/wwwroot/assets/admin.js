(function () {
  ...
  document.querySelectorAll('[data-affinity-question-editor]').forEach((editor) => {
    const type = editor.querySelector('[data-affinity-question-type]');
    const optionsContainer = editor.querySelector('[data-affinity-options-container]');
    const options = optionsContainer && optionsContainer.querySelector('textarea');
    const help = editor.querySelector('[data-affinity-type-help]');
    if (!type || !optionsContainer || !options) return;

    const refresh = () => {
      const isOpen = type.value === 'OpenEnded' || type.value === '2';
      optionsContainer.hidden = isOpen;
      options.disabled = isOpen;
      options.required = !isOpen;
