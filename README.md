...
1. 进入「后台 → 站点设置 → 邮件通知」，填写 SMTP Host、端口、加密方式、账号、密码或授权码、发件地址、站点公开地址及双方邮箱
2. 保存后分别点击「发送给男主 / 女主」验证连接、认证和投递
3. 双方各自进入「后台 → 通知」，在接收渠道中勾选 Email；Topic 偏好会同时作用于 Email 与 Web Push
